...
const criminalUsers = users.filter((e, i, arr)=>{
    if(!arr[i].email && !arr[i].phone)
        return arr[i];
});
console.log(criminalUsers);

const arr2 = users.map((ele, j, list)=>list[j].age);
// 가장 나이가 많은 유저들
const oldUsers = users.filter((e, i, arr)=>{
...
// 가장 나이가 어린 유저들
const youngUsers = users.filter((e, i, arr)=>{
...
const richUsers = users.filter((e, i, arr)=>{
    const arr3 = users.map((ele, j, list)=>(list[j].basket).length);
    let max = Math.max(...arr3);
    if((arr[i].basket).length == max)
        return arr[i];
});
console.log(richUsers);

// 한 해가 지난 후 나이 먹은 유저들
const nextYearUsers = users;
